...
from PCR_Jittor.jittor.modules.ops import apply_transform, pairwise_distance, get_rotation_translation_from_transform
from PCR_Jittor.jittor.utils.registration import compute_transform_mse_and_mae
...
def modified_chamfer_distance(raw_points, ref_points, src_points, gt_transform, transform, reduction='mean'):
    ...
    chamfer_distance_p_q = jt.sqrt(nn_sq_distances_p_q).mean(dim=-1)  # (B)

    # Q -> P_raw
    composed_transform = nn.matmul(transform, linalg.inv(gt_transform))  # (B, 4, 4)
    ...
    chamfer_distance_q_p = jt.sqrt(nn_sq_distances_q_p).mean(dim=-1)  # (B)
...
def relative_rotation_error(gt_rotations, rotations):
    ...
    """
    mat = nn.matmul(rotations.transpose(-1, -2), gt_rotations)
    trace = mat[..., 0, 0] + mat[..., 1, 1] + mat[..., 2, 2]
    x = 0.5 * (trace - 1.0)
    x = x.clamp(min_v=-1.0, max_v=1.0)
    x = jt.arccos(x)
    rre = 180.0 * x / np.pi
    return rre
...
def relative_translation_error(gt_translations, translations):
    ...
    Returns:
        rre (Tensor): relative rotation errors (*)
    """
    rte =jt.norm(gt_translations - translations, dim=-1)
    return rte
...
def anisotropic_transform_error(gt_transforms, transforms, reduction='mean'):
    ...
    gt_transforms_array = gt_transforms
    ...
    r_mse = np.array(all_r_mse)
    r_mae =np.array(all_r_mae)
    t_mse =np.array(all_t_mse)
    t_mae = np.array(all_t_mae)
