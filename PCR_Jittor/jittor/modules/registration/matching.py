...
from PCR_Jittor.jittor.modules.ops import index_select, apply_transform, pairwise_distance, get_point_to_node_indices
...
def extract_correspondences_from_scores(
    ...
    """
    score_mat = jt.exp(score_mat)
    ref_length, src_length = score_mat.shape

    ref_max_indices ,ref_max_scores = jt.argmax(score_mat, dim=1)
    ref_indices = jt.arange(ref_length)
    ref_corr_scores_mat = jt.zeros_like(score_mat)
    ref_corr_scores_mat[ref_indices, ref_max_indices] = ref_max_scores
    ref_corr_masks_mat = jt.greater(ref_corr_scores_mat, threshold)

    if mutual or bilateral:
        src_max_indices, src_max_scores = jt.argmax(score_mat, dim=0)
        src_indices = jt.arange(src_length)
        src_corr_scores_mat = jt.zeros_like(score_mat)
        src_corr_scores_mat[src_max_indices, src_indices] = src_max_scores
        src_corr_masks_mat =jt.greater(src_corr_scores_mat, threshold)

        if mutual:
            corr_masks_mat = jt.logical_and(ref_corr_masks_mat, src_corr_masks_mat)
        else:
            corr_masks_mat = jt.logical_or(ref_corr_masks_mat, src_corr_masks_mat)
    else:
    ...
    sel_indices = jt.nonzero(corr_masks_mat)
    ref_corr_indices=sel_indices[:,0].reshape(-1)
    src_corr_indices =sel_indices[:,1].reshape(-1)
...
def extract_correspondences_from_scores_threshold(
    scores_mat, threshold, has_dustbin= False, return_score= False
):
    ...
    Returns:
        ...
        corr_scores (Tensor): (C,)
    """
    scores_mat = jt.exp(scores_mat)
    if has_dustbin:
        scores_mat = scores_mat[:-1, :-1]
    masks = jt.greater(scores_mat, threshold)
    sel_indices = jt.nonzero(masks)
    ref_corr_indices=sel_indices[:,0].reshape(-1)
    src_corr_indices =sel_indices[:,1].reshape(-1)
...
def extract_correspondences_from_scores_topk(
    scores_mat, k, has_dustbin= False, largest= True, return_score= False
):
    ...
    if has_dustbin:
        ref_masks = jt.not_equal(ref_corr_indices, scores_mat.shape[0] - 1)
        src_masks = jt.not_equal(src_corr_indices, scores_mat.shape[1] - 1)
        masks = jt.logical_and(ref_masks, src_masks)
...
def dense_correspondences_to_node_correspondences(
    ...
    node_corr_indices, node_corr_counts = jt.unique(node_corr_indices, return_counts=True)
    ref_node_corr_indices = node_corr_indices // src_nodes.shape[0]
    src_node_corr_indices = node_corr_indices % src_nodes.shape[0]
    node_corr_indices = jt.stack([ref_node_corr_indices, src_node_corr_indices], dim=1)
...
def get_node_correspondences(
    ...
    if ref_masks is None:
        ref_masks = jt.ones(size=(ref_nodes.shape[0],), dtype='bool')
    if src_masks is None:
        src_masks = jt.ones(size=(src_nodes.shape[0],), dtype='bool')
    if ref_knn_masks is None:
        ref_knn_masks = jt.ones(size=(ref_knn_points.shape[0], ref_knn_points.shape[1]), dtype='bool')
    if src_knn_masks is None:
        src_knn_masks = jt.ones(size=(src_knn_points.shape[0], src_knn_points.shape[1]), dtype='bool')

    node_mask_mat = jt.logical_and(ref_masks.unsqueeze(1), src_masks.unsqueeze(0))  # (M, N)

    # filter out non-overlapping patches using enclosing sphere
    ref_knn_dists = jt.norm(ref_knn_points - ref_nodes.unsqueeze(1), dim=-1)  # (M, K)
    ref_knn_dists=jt.masked_fill(ref_knn_dists,jt.logical_not(ref_knn_masks), 0.0)
    ref_max_dists = ref_knn_dists.max(dim=1).reshape(-1)  # (M,)
    src_knn_dists = jt.norm(src_knn_points - src_nodes.unsqueeze(1), dim=-1)  # (N, K)
    src_knn_dists=jt.masked_fill(src_knn_dists,jt.logical_not(src_knn_masks), 0.0)
    src_max_dists = src_knn_dists.max(dim=1).reshape(-1)    # (N,)
    dist_mat = jt.sqrt(pairwise_distance(ref_nodes, src_nodes))  # (M, N)
    intersect_mat = jt.greater(ref_max_dists.unsqueeze(1) + src_max_dists.unsqueeze(0) + pos_radius - dist_mat, 0)
    intersect_mat = jt.logical_and(intersect_mat, node_mask_mat)
    sel_indices = jt.nonzero(intersect_mat)
    sel_ref_indices=sel_indices[:,0].reshape(-1)
    sel_src_indices =sel_indices[:,1].reshape(-1)
    ...
    point_mask_mat = jt.logical_and(ref_knn_masks.unsqueeze(2), src_knn_masks.unsqueeze(1))  # (B, K, K)

    # compute overlaps
    
    dist_mat = pairwise_distance(ref_knn_points, src_knn_points)  # (B, K, K)
    dist_mat=jt.masked_fill(dist_mat,jt.logical_not(point_mask_mat), 1e12)
    point_overlap_mat = jt.less(dist_mat, pos_radius ** 2)  # (B, K, K)
    ref_overlap_counts=point_overlap_mat.sum(-1)
    ref_overlap_counts=jt.not_equal(ref_overlap_counts,0)
    ref_overlap_counts=ref_overlap_counts.sum(-1).float()


    src_overlap_counts=point_overlap_mat.sum(-2)
    src_overlap_counts=jt.not_equal(src_overlap_counts,0)
    src_overlap_counts=src_overlap_counts.sum(-1).float()
    ...
    overlap_masks = jt.greater(overlaps, 0)
    ref_corr_indices = sel_ref_indices[overlap_masks]
    src_corr_indices = sel_src_indices[overlap_masks]
    corr_indices = jt.stack([ref_corr_indices, src_corr_indices], dim=1)
...
def node_correspondences_to_dense_correspondences(
    ...
    if ref_knn_masks is None:
        ref_knn_masks = jt.ones_like(ref_knn_indices)
    if src_knn_masks is None:
        src_knn_masks = jt.ones_like(src_knn_indices)
    ...
    dist_mat = jt.sqrt(pairwise_distance(ref_node_corr_knn_points, src_node_corr_knn_points))  # (P, K, K)
    corr_mat = jt.less(dist_mat, matching_radius)
    mask_mat = jt.logical_and(ref_node_corr_knn_masks.unsqueeze(2), src_node_corr_knn_masks.unsqueeze(1))
    corr_mat = jt.logical_and(corr_mat, mask_mat)  # (P, K, K)
    sel_indices = jt.nonzero(corr_mat)# (C,) (C,) (C,)
    batch_indices=sel_indices[:,0].reshape(-1)
    row_indices =sel_indices[:,1].reshape(-1)
    col_indices =sel_indices[:,2].reshape(-1)
...
def get_node_overlap_ratios(
    ...
    unique_ref_corr_indices = jt.unique(corr_indices[:, 0])
    unique_src_corr_indices = jt.unique(corr_indices[:, 1])
    ref_overlap_masks = jt.zeros(ref_points.shape[0] + 1)# pad for following indexing
    src_overlap_masks = jt.zeros(src_points.shape[0] + 1)  # pad for following indexing
