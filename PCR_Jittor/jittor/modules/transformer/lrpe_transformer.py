...
from PCR_Jittor.jittor.modules.transformer.positional_embedding import LearnablePositionalEmbedding
...
class LRPEMultiHeadAttention(nn.Module):
    ...
    def get_embeddings(self, q, emb_indices):
        emb_all_indices = jt.arange(self.num_embeddings).cuda()  # (P,)
        emb_bank = rearrange(self.embedding(emb_all_indices), 'p (h c) -> h p c', h=self.num_heads)
        attention_scores = linalg.einsum('bhnc,hpc->bhnp', q, emb_bank)
        emb_indices = emb_indices.unsqueeze(1).expand(-1, self.num_heads, -1, -1)  # (B, N, M) -> (B, H, N, M)
        attention_scores = jt.gather(attention_scores, dim=-1, index=emb_indices)  # (B, H, N, P) -> (B, H, N, M)
