...
class SinusoidalPositionalEmbedding(nn.Module):
    def __init__(self, d_model):
        ...
        div_indices = jt.arange(0, d_model, 2).float()
        self.div_term = jt.exp(div_indices * (-np.log(10000.0) / d_model)).stop_grad()
        #self.register_buffer('div_term', div_term)

    def execute(self, emb_indices):
        ...
        sin_embeddings = jt.sin(omegas)
        cos_embeddings = jt.cos(omegas)
        embeddings = concat([sin_embeddings, cos_embeddings], 2)  # (-1, d_model/2, 2)
...
class LearnablePositionalEmbedding(nn.Module):
    ...
    def execute(self, emb_indices):
        ...
        max_emd_indices = jt.full_like(emb_indices, self.num_embeddings - 1)
        emb_indices = jt.minimum(emb_indices, max_emd_indices)
