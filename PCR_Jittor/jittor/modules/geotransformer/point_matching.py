...
class PointMatching(nn.Module):
    ...
    def compute_correspondence_matrix(self, score_mat, ref_knn_masks, src_knn_masks):
        ...
        batch_indices = jt.arange(batch_size).cuda()
        ...
        ref_indices = jt.arange(ref_length).cuda().view(1, ref_length, 1).expand(batch_size, -1, self.k)  # (B, N, K)
        ...
        src_indices = jt.arange(src_length).cuda().view(1, 1, src_length).expand(batch_size, self.k, -1)  # (B, K, N)
