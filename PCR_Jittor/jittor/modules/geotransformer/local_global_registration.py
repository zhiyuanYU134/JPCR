...
from PCR_Jittor.jittor.modules.ops import apply_transform
from PCR_Jittor.jittor.modules.registration import WeightedProcrustes
from PCR_Jittor.jittor.modules.ops import index_select
import torch

class LocalGlobalRegistration_jittor(nn.Module):
    def __init__(
        self,
        k: int,
        ...
        """
        super(LocalGlobalRegistration_jittor, self).__init__()
        self.k = k
    ...
    def compute_correspondence_matrix(self, score_mat, ref_knn_masks, src_knn_masks):
        ...
        mask_mat = jt.logical_and(ref_knn_masks.unsqueeze(2), src_knn_masks.unsqueeze(1))
        ...
        ref_indices = jt.arange(ref_length).view(1, ref_length, 1).expand(batch_size, -1, self.k)  # (B, N, K)
        ref_score_mat = jt.zeros_like(score_mat)
        ref_score_mat[ref_batch_indices, ref_indices, ref_topk_indices] = ref_topk_scores
        ref_corr_mat = jt.greater(ref_score_mat, self.confidence_threshold)
        ...
        src_indices = jt.arange(src_length).view(1, 1, src_length).expand(batch_size, self.k, -1)  # (B, K, N)
        src_score_mat = jt.zeros_like(score_mat)
        src_score_mat[src_batch_indices, src_topk_indices, src_indices] = src_topk_scores
        src_corr_mat = jt.greater(src_score_mat, self.confidence_threshold)

        # merge results from two sides
        if self.mutual:
            corr_mat = jt.logical_and(ref_corr_mat, src_corr_mat)
        else:
            corr_mat = jt.logical_or(ref_corr_mat, src_corr_mat)

        if self.use_dustbin:
            corr_mat = corr_mat[:, -1:, -1]

        corr_mat = jt.logical_and(corr_mat, mask_mat)
    ...
    def convert_to_batch(ref_corr_points, src_corr_points, corr_scores, chunks):
        ...
        indices = concat([jt.arange(x, y) for x, y in chunks], 0)
        ...
        indices = concat([jt.arange(x, y) for x, y in target_chunks], 0)
        indices0 = indices.unsqueeze(1).expand(indices.shape[0], 3)  # (total,) -> (total, 3)
        indices1 = jt.arange(3).unsqueeze(0).expand(indices.shape[0], 3)# (3,) -> (total, 3)

        
        max_corr=int(max_corr)

        batch_ref_corr_points = jt.zeros([batch_size * max_corr, 3])
        batch_src_corr_points = jt.zeros([batch_size * max_corr, 3])
        batch_corr_scores = jt.zeros(batch_size * max_corr)

        batch_ref_corr_points[indices]=ref_corr_points
        batch_src_corr_points[indices]=src_corr_points
        batch_corr_scores[indices]=corr_scores
        
        """ batch_ref_corr_points=jt.index_fill_(batch_ref_corr_points,0,indices, ref_corr_points)
        batch_src_corr_points=jt.index_fill_(batch_src_corr_points,0,indices, src_corr_points)
        batch_corr_scores=jt.index_fill_(batch_corr_scores,0,indices, corr_scores) """
    ...
    def recompute_correspondence_scores(self, ref_corr_points, src_corr_points, corr_scores, estimated_transform):
        ...
        corr_residuals = jt.norm(ref_corr_points - aligned_src_corr_points, dim=1)
        inlier_masks = jt.less(corr_residuals, self.acceptance_radius)
    ...
    def local_to_global_registration(self, ref_knn_points, src_knn_points, score_mat, corr_mat):
        ...
        ref_indices =sel_indices[:,1].reshape(-1)
        src_indices =sel_indices[:,2].reshape(-1)

        #batch_indices, ref_indices, src_indices = jt.nonzero(corr_mat)
        ...
        unique_masks = jt.not_equal(batch_indices[1:], batch_indices[:-1])
        unique_indices = jt.nonzero(unique_masks).reshape(-1)+ 1
        ...
        if batch_size > 0:
            ...
            batch_transforms,src_points_centered,weights,ref_points_centered = self.procrustes(batch_src_corr_points, batch_ref_corr_points, batch_corr_scores)
            batch_aligned_src_corr_points = apply_transform(src_corr_points.unsqueeze(0), batch_transforms)
            batch_corr_residuals = jt.norm(
                ref_corr_points.unsqueeze(0) - batch_aligned_src_corr_points, dim=2
            )
            batch_inlier_masks = jt.less(batch_corr_residuals, self.acceptance_radius)  # (P, N)
            best_index,best_value = batch_inlier_masks.sum(dim=1).argmax(0)


            cur_corr_scores = corr_scores * batch_inlier_masks[best_index].float()
            cur_corr_scores=cur_corr_scores.reshape(-1)
        else:
        ...
        return global_ref_corr_points, global_src_corr_points, global_corr_scores, estimated_transform,src_points_centered,weights,ref_points_centered
    ...
    def execute(
        ...
        """
        score_mat=torch.from_numpy(score_mat.cpu().numpy()).float().cuda()
        score_mat = torch.exp(score_mat)
        score_mat=jt.Var(score_mat.cpu().numpy())
        ...
        ref_corr_points, src_corr_points, corr_scores, estimated_transform,batch_ref_corr_points, batch_src_corr_points, batch_corr_scores = self.local_to_global_registration(
            ref_knn_points, src_knn_points, score_mat, corr_mat
        )#

        return ref_corr_points, src_corr_points, corr_scores, estimated_transform,score_mat_ori,batch_ref_corr_points, batch_src_corr_points, batch_corr_scores
