...
from PCR_Jittor.jittor.modules.kpconv.functional import maxpool, nearest_upsample, global_avgpool, knn_interpolate
from PCR_Jittor.jittor.modules.kpconv.kpconv import KPConv_pure as KPConv
import time
...
class KNNInterpolate(nn.Module):
    ...
    def execute(self, s_feats, q_points, s_points, neighbor_indices):
        if self.k == 1:
...
class MaxPool(nn.Module):
    @staticmethod
    def execute(s_feats, neighbor_indices):
        return maxpool(s_feats, neighbor_indices)


class GlobalAvgPool(nn.Module):
    @staticmethod
    def execute(feats, lengths):
...
class ResidualBlock(nn.Module):
    def __init__(
        ...
        self.KPConv = KPConv(mid_channels, mid_channels, kernel_size, radius, sigma)
        if layer_norm:
